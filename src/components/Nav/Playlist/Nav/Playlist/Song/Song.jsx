    ...
    return (
        <tr className={Style.container}>
            ...
            <td className={Style.flex} onClick={() => setPlay(!play)} >
                <iframe src={`https://open.spotify.com/embed?uri=${song.track ? song.track.uri : ''}`} width={play ? '80' : '470'} height="80" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
                <div className={Style.containerLeft}>
                    <p style={{ display: song.track && play ? 'flex' : 'none' }} className={Style.songName}>{song.track ? song.track.name : ''}</p>
                    <div className={Style.inner}>
                        <p className={Style.e} style={{ display: song.track && play ? song.track.explicit ? 'flex' : 'none' : 'none' }}>E</p>
                        <a href={`/artist/${song.track && song.track.artists && song.track.artists[0] ? song.track.artists[0].id : ''}/${localStorage.getItem('token')}`} style={{ textDecoration: 'none' }} className={Style.artistName}>
                            {song.track ? song.track.artists.map(artist => <span style={{ display: song.track && play ? 'inline-block' : 'none' }} className={Style.artistName}>{artist.name}, </span>) : ''}
                        </a>
                    </div>
                </div>
            </td>
